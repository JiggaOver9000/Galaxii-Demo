...
class OutdatedSubState extends MusicBeatState
{
	...
	{
		super.create();
		var bg:FlxSprite = new FlxSprite().loadGraphic(Paths.image('studiosweek5prototype', 'shared'));
		bg.scale.x *= 1.55;
		bg.scale.y *= 1.55;
		bg.screenCenter();
		add(bg);
		...
		var txt:FlxText = new FlxText(0, 0, FlxG.width,
			"WARNING! "
			+ "\nThis is a DEMO"
			+ "\nNot all things are final and not all songs are included."
			+ "\nHeres whats included! (and what's not)"
			+ "\nIN THE DEMO: 1 Week, 5 Songs"
			+ "\nIN THE FULL VERSION: 2 Weeks, 12+ Songs, 2 Epilogue Weeks"
			+ "\nMake sure to leave a like on the gamebanana page!",
			32);
